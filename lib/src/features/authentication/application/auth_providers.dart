...
class Auth extends _$Auth {
  @override
  Future<PbAuth?> build() async {
    return await _fetch();
  }

  Future<PbAuth?> refresh() async {
    ...
    try {
      final token = await ref
          ...
          .readData();
      state = AsyncValue.data(PbAuth(token: token));
      return state.value;
    } catch (e) {
  ...
  Future<PbAuth?> _fetch() async {
    ...
    state = AsyncValue.data(PbAuth(token: result.value));
    return state.value;
  }
}
